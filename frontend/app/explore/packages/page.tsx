            ...
            <div className="space-y-1">
              {p.departure_city && (
                <p className="text-xs text-slate-500">
                  <span className="font-medium text-slate-400">From:</span> {p.departure_city}
                </p>
              )}
              <p className="text-xs text-slate-500">
      ...
      {expanded && (
        <div className="border-t border-slate-800 p-5 space-y-6">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {p.difficulty && (
              <span className="text-xs px-3 py-1.5 rounded-full bg-slate-800 text-slate-300 capitalize">
                💪 {p.difficulty}
              </span>
            )}
            {(p.group_min || p.group_max) && (
              <span className="text-xs px-3 py-1.5 rounded-full bg-slate-800 text-slate-300">
                👥 {p.group_min}–{p.group_max} people
              </span>
            )}
            {(p.age_min || p.age_max) && (
              <span className="text-xs px-3 py-1.5 rounded-full bg-slate-800 text-slate-300">
                🎂 Ages {p.age_min ?? "any"}–{p.age_max ?? "any"}
              </span>
            )}
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <div className="bg-slate-800 rounded-xl p-3 text-center">
                ...
                <p className="text-lg font-bold" style={{ color: "#d4a017" }}>₹{p.price_per_person?.toLocaleString()}</p>
              </div>
              {p.price_couple && (
                <div className="bg-slate-800 rounded-xl p-3 text-center">
                  <p className="text-xs text-slate-500 mb-1">Couple</p>
                  <p className="text-lg font-bold text-white">₹{p.price_couple?.toLocaleString()}</p>
                </div>
              )}
              {p.price_child && (
                <div className="bg-slate-800 rounded-xl p-3 text-center">
                  <p className="text-xs text-slate-500 mb-1">Child</p>
                  <p className="text-lg font-bold text-white">₹{p.price_child?.toLocaleString()}</p>
                </div>
              )}
            </div>
          </div>
          ...
          {p.available_dates?.length > 0 && (
            <div>
              <h3 className="text-sm font-bold text-white mb-2">📅 Available Dates</h3>
              <div className="flex flex-wrap gap-2">
                {p.available_dates.map((d: string, i: number) => (
                  <span key={i} className="text-xs px-3 py-1 rounded-full border border-slate-700 text-slate-300">
          ...
          {p.highlights?.length > 0 && (
            <div>
              <h3 className="text-sm font-bold text-white mb-2">⭐ Highlights</h3>
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                {p.highlights.map((h: string, i: number) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-slate-300">
                    <span className="mt-0.5 text-[#d4a017] shrink-0">•</span>{h}
                  </li>
                ))}
              </ul>
            </div>
          ...
          {p.itinerary?.length > 0 && (
            <div>
              <h3 className="text-sm font-bold text-white mb-3">🗺️ Day-by-Day Itinerary</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {p.exclusions?.length > 0 && (
              <div>
                <h3 className="text-sm font-bold text-white mb-2">❌ Exclusions</h3>
                <ul className="space-y-1">
          ...
          {p.meeting_point && (
            <div className="bg-slate-800/60 border border-slate-700 rounded-xl p-4">
              <h3 className="text-sm font-bold text-white mb-1">📍 Meeting Point</h3>
              <p className="text-xs text-slate-300 leading-relaxed">{p.meeting_point}</p>
            </div>
          )}
          ...
          {p.special_notes && (
            <div className="bg-amber-950/30 border border-amber-900/40 rounded-xl p-4">
              <h3 className="text-sm font-bold text-amber-300 mb-1">📌 Important Notes</h3>
              <p className="text-xs text-amber-200/80 leading-relaxed">{p.special_notes}</p>
            </div>
          )}
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {p.booking_phone && (
                <a href={`tel:${p.booking_phone}`}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold border border-slate-700 text-white hover:border-slate-500 transition-all">
                  📞 Call Agent
                </a>
              )}
              {p.booking_email && (
